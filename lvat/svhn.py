...
def maybe_download_and_extract():
    if not os.path.exists(FLAGS.data__dir):
        os.makedirs(FLAGS.data__dir)
    filepath_train_mat = os.path.join(FLAGS.data__dir, 'train_32x32.mat')
    filepath_test_mat = os.path.join(FLAGS.data__dir, 'test_32x32.mat')
    ...
    train_data = loadmat(FLAGS.data__dir + '/train_32x32.mat')
    ...
    test_data = loadmat(FLAGS.data__dir + '/test_32x32.mat')
    ...
    np.save('{}/train_images'.format(FLAGS.data__dir), train_x)
    np.save('{}/train_labels'.format(FLAGS.data__dir), train_y)
    np.save('{}/test_images'.format(FLAGS.data__dir), test_x)
    np.save('{}/test_labels'.format(FLAGS.data__dir), test_y)


def load_svhn():
    maybe_download_and_extract()
    train_images = np.load('{}/train_images.npy'.format(FLAGS.data__dir)).astype(np.float32)
    train_labels = np.load('{}/train_labels.npy'.format(FLAGS.data__dir)).astype(np.float32)
    test_images = np.load('{}/test_images.npy'.format(FLAGS.data__dir)).astype(np.float32)
    test_labels = np.load('{}/test_labels.npy'.format(FLAGS.data__dir)).astype(np.float32)
    return (train_images, train_labels), (test_images, test_labels)


def prepare_dataset():
    (train_images, train_labels), (test_images, test_labels) = load_svhn()
    dirpath = os.path.join(FLAGS.data__dir, 'seed' + str(FLAGS.dataset_seed))
...
def inputs(batch_size=100,
    ...
    filename_queue = generate_filename_queue(filenames, FLAGS.data__dir, num_epochs)
...
def unlabeled_inputs(batch_size=100,
    ...
    filenames = [os.path.join('seed' + str(FLAGS.dataset_seed), filename) for filename in filenames]
    filename_queue = generate_filename_queue(filenames, data_dir=FLAGS.data__dir)
    image, label = read(filename_queue)
...
if __name__ == "__main__":
    ...
    tf.app.flags.DEFINE_string('data__dir', './data/svhn', "")
