...
def load_cifar10():
    ...
    dest_directory = FLAGS.data__dir
    ...
    for i, data_fn in enumerate(
            sorted(glob.glob(FLAGS.data__dir + '/cifar-10-batches-py/data_batch*'))):
    ...
    test = unpickle(FLAGS.data__dir + '/cifar-10-batches-py/test_batch')
    ...
    np.save('{}/components'.format(FLAGS.data__dir), components)
    np.save('{}/mean'.format(FLAGS.data__dir), mean)
...
def unlabeled_inputs(batch_size=100,
    ...
    filenames = [os.path.join('seed' + str(FLAGS.dataset_seed), filename) for filename in filenames]
    filename_queue = generate_filename_queue(filenames, FLAGS.data__dir)
    image, label = read(filename_queue)
...
if __name__ == "__main__":
    ...
    #tf.app.flags.DEFINE_string('data__dir', '/data/img/CIFAR10__labled_4000',
    #tf.app.flags.DEFINE_string('data__dir', '/data/img/CIFAR10_w_ZCA__labled_4000',
    tf.app.flags.DEFINE_string('data__dir', '/data/img/CIFAR10_w_ZCA',
